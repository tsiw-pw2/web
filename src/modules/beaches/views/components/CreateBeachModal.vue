...
function onProceed() {
    ...
    if (!n || !m || !d) return
    emit("create", { name: n, municipality: m, district: d })
    close()
}
...
<template>
     <ModalRoot v-model="open" ariaLabelledby="create-beach-title"
        ...
        <form class="flex flex-col gap-3" @submit.prevent="onProceed">
            ...
            <div class="flex flex-col gap-1"> <FieldLabel required>Nome</FieldLabel> <Input v-model="name" class="w-full" placeholder="Nome da praia" /> </div>
