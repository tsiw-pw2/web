<script setup lang="ts">
	...
	import { districtCodeFromMunicipalityLabel } from "@/modules/campaigns/services/campaignsList"
	...
	function syncFromCampaign() {
		const c = props.campaign
		if (!c) return
		title.value = c.title
		district.value = districtCodeFromMunicipalityLabel(c.municipality)
		meetingTime.value = ""
		startDate.value = c.startDate
		endDate.value = c.endDate === "—" ? "" : c.endDate
		status.value = "scheduled"
		information.value = c.beach !== "—" ? c.beach : ""
	}
	...
	watch(
		() => [open.value, props.campaign] as const,
		([isOpen]) => {
			if (isOpen) syncFromCampaign()
		},
		{ immediate: true },
	)
</script>
...
<template>
	<ModalRoot v-model="open" ariaLabelledby="edit-campaign-title">
		<div class="flex items-start justify-between gap-4">
			<h3 id="edit-campaign-title" class="text-lg font-semibold leading-7 text-neutral-950">
				Editar campanha
			</h3>
