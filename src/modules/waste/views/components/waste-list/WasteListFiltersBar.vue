...
import { WASTE_UNIT_SELECT_OPTIONS } from "@/modules/waste/lib/wasteUnitFilter"
...
const units = defineModel<WasteUnitKey[]>("units", { required: true })
...
<template>
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
        ...
        <div class="flex min-w-0 flex-col gap-3 sm:flex-row sm:shrink-0 sm:gap-3">
            <MultiSelect
                id="waste-filter-unit"
                v-model="units"
                class="w-full sm:w-[200px]"
                filter-mode
                :options="WASTE_UNIT_SELECT_OPTIONS"
                placeholder="Unidade"
                empty-label="Todas as unidades"
                clear-label="Limpar unidades"
            />
            <Select
