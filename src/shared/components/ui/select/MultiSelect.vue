...
const emptyLabelRef = toRef(props, "emptyLabel")
...
function syncHighlightToValue() {
    ...
    const selectedIdx = opts.findIndex((o) => selected.includes(o.value) && !o.disabled)
    if (selectedIdx >= 0) {
...
function toggleIndex(i: number) {
    ...
    const at = next.indexOf(opt.value)
...
function onTriggerKeydown(e: KeyboardEvent) {
    ...
    if (e.key === "Enter" || e.key === " ") {
        e.preventDefault()
        if (open.value) toggleIndex(highlightedIndex.value)
...
function onPanelKeydown(e: KeyboardEvent) {
    ...
    if (e.key === "Enter" || e.key === " ") {
        e.preventDefault()
        toggleIndex(highlightedIndex.value)
    }
}
...
const displayLabel = computed(() => {
    ...
    if (selected.length === 0) {
        return emptyLabelRef.value || null
    }
...
const labelClass = computed(() =>
    selectLabelClasses(
        ...
        placeholderRef.value.length > 0 || emptyLabelRef.value.length > 0,
        disabled.value === true,
    ),
)
...
const showClear = computed(
    () => props.filterMode === true && modelValue.value.length > 0,
)
